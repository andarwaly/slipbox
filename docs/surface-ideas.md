...
4. **Dismiss at surface time** — some candidates aren't worth keeping at all (too thin, too redundant). These are dropped silently, leaving no trace — different from a candidate dismissed later during `ground-claim`'s pick step, which does get recorded.
...
`ground-claim` reads what this skill writes, and can run in a separate session entirely.
