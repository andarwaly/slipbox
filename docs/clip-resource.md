...
1. **Take the URL** — a single article or video link, checked to be a plain `http`/`https` URL before it reaches a fetch command. No pasted-text fallback; if you hand it raw text, it asks for a URL instead.
