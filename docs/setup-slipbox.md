...
Setup reports cache inventory counts for compatible, missing, incompatible, older-compatible, unresolved-source, and orphaned entries. In an existing vault with Literature notes but no cache, it reads each complete Resource first and builds source-owned maps from the source, never from selective Literature content. It reconciles Core Idea, Source Points (or legacy Key Claims), Key Concepts, Mentioned, and Open Questions to source units with private statuses: matched, matched-with-qualification-risk, matched-to-multiple-units, unmatched, or source-support-unclear. Missing Resources block reconciliation and are reported; note semantics are not rewritten automatically. It offers cache choices—build missing + incompatible, a chosen scope, refresh all, or defer—separately from note-format migration choices: detect exact-heading `Key Claims` → `Source Points` records in Literature and prefixed H1s in Reference and Evergreen; choose all-valid, selected, lazy/on first access, or defer. Lazy records authorization but performs each compatible mechanical repair only when that note is subsequently opened by a slipbox workflow; defer records no authorization. Unusual structures are skipped and reported. Reference migration audits prefixed H1s, legacy term/reference fields, aliases and provenance duplicates, Literature links in `sources`, unresolved sources, wrong Literature→Reference edges, and configured zone/type drift. Resolvable records are normalized to clean H1s and deduplicated original Resource provenance, while unresolved structures are skipped and reported. Mechanical repair uses append-only ledger tombstones plus corrected Resource→Reference edges. Semantic migration is scoped separately and classifies removed material rather than silently deleting or reclassifying it. It asks for Git policy (`off`, `ask` recommended, or `auto`) only when a repository is detected; no Git-detection boolean is stored. The config schema requires `git` and `cache` objects and rejects tracked `.slipbox/work/`.

Mixed Reference provenance may repair resolvable Resources while retaining unresolved values as blocked repairs; those do not block unrelated values unless identity or definition is ambiguous.
