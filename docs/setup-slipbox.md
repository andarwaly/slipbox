...
Setup reports cache inventory counts for compatible, missing, incompatible, older-compatible, unresolved-source, and orphaned entries. It offers build missing + incompatible, build a chosen scope, refresh all, or defer. Cache migration and note-format migration are authorized independently. It asks for Git policy (`off`, `ask` recommended, or `auto`) only when a repository is detected; no Git-detection boolean is stored. The config schema requires `git` and `cache` objects and rejects tracked `.slipbox/work/`.
