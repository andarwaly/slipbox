...
Setup reports cache inventory counts for compatible, missing, incompatible, older-compatible, unresolved-source, and orphaned entries. In an existing vault with Literature notes but no cache, it reads each complete Resource first and builds source-owned maps from the source, never from selective Literature content. It reconciles Core Idea, Source Points (or legacy Key Claims), Key Concepts, Mentioned, and Open Questions to source units with private statuses: matched, matched-with-qualification-risk, matched-to-multiple-units, unmatched, or source-support-unclear. Missing Resources block reconciliation and are reported; note semantics are not rewritten automatically. It offers cache choices—build missing + incompatible, a chosen scope, refresh all, or defer—separately from exact-heading `Key Claims` → `Source Points` migration choices (selected, all compatible, or defer). Unusual structures are skipped and reported. Any later Reference migration is a third, independent authorization. It asks for Git policy (`off`, `ask` recommended, or `auto`) only when a repository is detected; no Git-detection boolean is stored. The config schema requires `git` and `cache` objects and rejects tracked `.slipbox/work/`.
