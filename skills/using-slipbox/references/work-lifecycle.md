...
Source, target, affected, and mutation paths are relative to the vault root, one level above the installed `.slipbox/` directory, regardless of the calling directory. Internal mutation paths include the `.slipbox/` prefix (for example, `.slipbox/links.jsonl`); replacement paths remain relative to the selected work directory. Git configuration is read from `.slipbox/config.json`.
...
For Reference synthesis, checkpoint the reconciled map and explanatory draft at admission and again after any ambiguity or conflict is resolved. An `extend-provenance` operation keeps existing body bytes stable when a new source adds warrant only; it stages provenance and the Resource→Reference edge. A `recompose` operation stages only the explanatory sections required by a material definition, characteristic, or inquiry-relevant content change and preserves unrelated sections. Re-read and fingerprint the target immediately before staging. Any concurrent target change blocks finalization; it never overwrites the user's newer synthesis. The synthesis map is transient and is not published.
