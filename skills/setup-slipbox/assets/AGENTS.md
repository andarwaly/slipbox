...
- `write-reference` — synthesizes a Reference note from the literature notes that already wikilink to it. Never runs `/grounding` itself; grounding already happened upstream, at the claim level.
- `find-connections` — scans existing notes, in one of two modes. `--references` surfaces Reference/Person/Location/Organization recurrence for the user to act on with `write-reference`. `--evergreen` surfaces missing links and sparked ideas, writing mechanical links directly and routing sparked ideas to the evergreen backlog.
