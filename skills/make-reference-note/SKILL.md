...
- Where characterizations conflict, surface the conflict to the user rather than
  ...
  reason, and origin paths `/using-slipbox`. Use `origin_kind: note-connection` and
  every actual Literature-note path whose grounded characterization participates in
  the conflict. Never use the Reference output path or reconstruct paths from config.
