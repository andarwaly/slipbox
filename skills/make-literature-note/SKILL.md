...
Before the first Literature publication, use `origin_kind: source` with the actual
vault-relative Resource path. After the Literature file exists, use
`origin_kind: literature-note` with that exact saved path. Never reconstruct either
path from configured names or prefixes.
...
Use `origin_kind: literature-note` with the exact saved Literature path. If no
Literature file exists yet, retain the actual Resource path with `origin_kind: source`.
...
literature-note paths as origin paths and set the matching `origin_kind`. There is no
mandatory reaction prompt.
